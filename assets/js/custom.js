...
var twebuTable = $('.twebuTables').DataTable({
    ...
    "fnRowCallback": function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
        var imgLink = aData['status'];
        if (imgLink == '1') {
            $('td:eq(9)', nRow).html('Active');
        } else {
            $('td:eq(9)', nRow).html('Block');
        }
        return nRow;
    },
    "columns": [
...
$(".categoryTables").on('click', '.edit-category', function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#allow_per").val(category.use_permission);
        },
        error: function (data) {
            console.log('unable to send request..');
        }
...
$('#support_subject').change(function () {
    var subj_id = $('#support_subject').val();
    $.ajax({
        url: base_url + 'admin_master/get_subcode',
        method: 'post',
        data: {
            sid: subj_id,
        },
        dataType: 'json',
        success: function (response) { 
             $('#subject_code').val(response.codes);
        }
    });
});

$('#getsupport_msubject').change(function () {
    var board_id = $('#getsupport_msubject').val();
    $.ajax({
        url: base_url + 'admin_master/get_subm',
        method: 'post',
        data: {
        ...
        dataType: 'json',
        success: function (response) {
            $('#getsupport_subject').find('option').not(':first').remove();
            $.each(response, function (index, data) {
                $('#getsupport_subject').append('<option value="' + data['id'] + '">' + data['name'] + '</option>');
            });
        }
    });
});
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $('#select_subject')
                    ...
                    .remove()
                    .end()
                    .append('<option value="" disabled>Select Book</option>')
                    .val('')
                    ;
            $('#select_subject').find('option').not(':first').remove();
            $.each(response, function (index, data) {
...
/* student and teacher password validation start*/
...
$('#teacher_password, #teacher_cpassword').on('keyup', function () {
    if ($('#teacher_password').val() == $('#teacher_cpassword').val()) {
        $('#tmessage').html('Matching').css('color', 'green');
    } else 
        $('#tmessage').html('Not Matching').css('color', 'red');
});
/* end validation */

/* progress bar start */
$(document).ready(function() { 
       $('#imageupload').submit(function(e) {                   // form id
        if($('#support_image').val()) {       					// file id
          e.preventDefault();

          $("#progress-bar-status-show").width('0%');
          var file_details    =   document.getElementById("support_image").files[0];
          var extension       =   file_details['name'].split(".");
          
          var allowed_extension   =   ["gif", "jpg", "jpeg", "png", "mp4", "3gp", "flv", "svg", "webp", "wmv", "ico", "xlsx", "doc", "docx", "pdf", "zip", "rar"];
